...
const ChefPostingSystem = () => {
    ...
    const fetchMeals = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          const normalized = data.map((meal, index) => {
            const originKey = meal.originKey || "generic";
            const origin =
              ORIGIN_COORDS[originKey] || ORIGIN_COORDS["generic"];

            return {
              // server fields
              ...meal,
              // normalize for frontend expectations
              id: meal._id || meal.id || index,
              title: meal.title || "Untitled Meal",
              description: meal.description || "",
              price: meal.price ?? 0,
              servings: meal.servings ?? 1,
              servingsLeft:
                meal.servingsLeft ?? meal.servings ?? 1,
              tags: Array.isArray(meal.tags) ? meal.tags : [],
              culturalNote: meal.culturalNote || "",
              chef: meal.chef || meal.chefName || "Campus Chef",
              chefBio: meal.chefBio || "Student chef",
              dorm: meal.dorm || "Unknown dorm",
              image:
                meal.image ||
              ...
              originKey,
              lat: meal.lat ?? origin.lat,
              lng: meal.lng ?? origin.lng,
            };
          });

          setMeals(normalized);
          setFilteredMeals(normalized);
          return;
        }

        // If backend returns empty array -> fallback to sample data
  ...
  const handleBuyTicket = (meal) => {
    if (meal.servingsLeft > 0) {
      // purely frontend for now (no PATCH route in backend)
      setMeals((prev) =>
        prev.map((m) =>
          m.id === meal.id
            ? { ...m, servingsLeft: m.servingsLeft - 1 }
            : m
        )
      );
      const note = {
        id: Date.now(),
        message: `Successfully reserved ${meal.title}!`,
        time: new Date(),
      };
  ...
  const CreatePostView = () => {
    ...
    const submit = async () => {
      ...
      try {
        ...
        console.error(
          "Error saving meal to backend, adding locally instead:",
          err
        );
        const fallbackMeal = {
          ...payload,
        ...
        setMeals((prev) => [fallbackMeal, ...prev]);
        setShowCreatePost(false);
        alert("Meal posted locally (backend error).");
      }
    };
