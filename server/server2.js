...
import fs from "fs";
import generateRoutes from "./generateRoutes.js"; // router for other gen features
...
// --- Static serving for uploads (images + audio) ---
app.use("/uploads", express.static(path.join(__dirname, "uploads")));

// --- Mount generate routes (other generation endpoints) ---
...
const mealSchema = new mongoose.Schema(
  {
    ...
    // ⭐ Generated dish history (Gemini + Audio)
    generatedHistory: {
      text: String,
      audioUrl: String,
...
app.post("/api/upload", upload.single("image"), (req, res) => {
  ...
  // Absolute URL so the frontend can use it directly
...
app.post("/api/users", async (req, res) => {
  try {
    ...
    if (!name || !email || !dorm || !role) {
      return res
        .status(400)
        .json({ error: "Missing required fields: name, email, dorm, role" });
    }
...
// --------------------------------------------------
// ⭐ GENERATE DISH HISTORY (Gemini + ElevenLabs)
// --------------------------------------------------
